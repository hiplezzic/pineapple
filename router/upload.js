...
function Upload (mysqlConnection, upload, ejsPine) {
	...
	this.router.post('/test', function (req, res, next) {
		var nickname = req.session.passport.user.nickname;
		var query = 'INSERT INTO youtube (title, url, uploader) VALUES (\'' + req.body.title +'\',\''+ req.body.url +'\',\''+ nickname +'\')';
